...
const EditPiratesForm = () => {
  const { id } = useParams();
  const { getPirates, updatePirates } = Apipirates();
  ...
  useEffect(() => {
    const fetchPirate = async () => {
      try {
        ...
        setName(res.name);
        setImg(res.img);
        setTreasure(res.treasure);
        setPhrase(res.phrase);
        setPosition(res.position);
        setPegLeg(res.pegLeg);
        setEyePatch(res.eyePatch);
        setHookHand(res.hookHand);
      } catch (error) {
        console.log(error);
        navigate("/pirates");
      }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await updatePirates(id, {
        name,
        img,
        treasure,
        phrase,
      ...
      <header className="bg-[#b45f06] h-28 text-[#4a3a2b] font-bold flex justify-end pr-18">
        <div className="flex items-center justify-between w-1/2 mr-14">
          <p className="text-4xl">
            Update <span className="text-orange-600">{pirate.name || ""}</span>{" "}
          </p>
          <SeeDetailsComponent id={id} />
        </div>
      </header>
      <form
        action=""
...
export default EditPiratesForm;
